...
@section('css')
    <link rel="stylesheet" href="{{ asset('backend/assets/css/attendance.css') }}">
    <style>
        .switch-toggle {
            width: auto;
        }
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('attendances.update', ['date' => $attendances[0]->date]) }}"
                                method="post" id="myForm">
                                @csrf
                                @method('patch')
                                <div class="form-group col-md-4">
                                    <label for="date" class="control-label">Attendance Date</label>
                                    <input type="date" name="date" id="date" value="{{ $attendances[0]->date }}"
                                        class="checkdate form-control form-control-sm singledatepicker"
                                        placeholder="Attendance Date" autocomplete="off" disabled>
                                </div>
                                    ...
                                    <tbody>
                                        @foreach ($attendances as $key => $attendance)
                                            <tr id="div {{ $attendance->id }}" class="text-center">
                                                <input type="hidden" name="attendances[{{ $key }}][id]"
                                                    value="{{ $attendance->id }}">
                                                <input type="hidden" name="attendances[{{ $key }}][employee_id]"
                                                    value="{{ $attendance->id }}" class="employee_id">
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $attendance->employee->name }}</td>
                                                <td colspan="3">
                                                    <div class="switch-toggle switch-3 switch-candy">
                                                        <input class="present" id="present[{{ $key }}]"
                                                            name="attendances[{{ $key }}][status]" value="Present"
                                                            type="radio"
                                                            {{ $attendance->status == 'Present' ? 'checked' : '' }}>
                                                        <label for="present[{{ $key }}]">Present</label>

                                                        <input class="leave" id="leave[{{ $key }}]"
                                                            name="attendances[{{ $key }}][status]" value="Leave"
                                                            type="radio"
                                                            {{ $attendance->status == 'Leave' ? 'checked' : '' }}>
                                                        <label for="leave[{{ $key }}]">Leave</label>

                                                        <input class="absent" id="absent[{{ $key }}]"
                                                            name="attendances[{{ $key }}][status]" value="Absent"
                                                            type="radio"
